...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Komponen Logo Infinity Cloud (Warna otomatis ngikutin text)
  const InfinityCloudLogo = ({ className }: { className?: string }) => (
    <svg className={className} viewBox="0 0 100 60" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M 25 45 C 10 45 10 20 25 20 C 35 20 40 30 50 30 C 60 30 65 20 75 20 C 90 20 90 45 75 45 C 65 45 60 35 50 35 C 40 35 35 45 25 45 Z" stroke="currentColor" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="50" cy="32.5" r="5" fill="currentColor" />
    </svg>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      {/* SIDEBAR DESKTOP: Sembunyi di HP, muncul di Laptop */}
      <aside className={`hidden md:flex ${isCollapsed ? 'w-20' : 'w-72'} bg-[#2A0510] text-white flex-col shadow-xl z-10 transition-all duration-300 ease-in-out`}>
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <div className={`flex items-center gap-3 overflow-hidden whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
            <InfinityCloudLogo className="w-8 h-8 shrink-0 text-white" />
            <div>
              <h1 className="text-xl font-bold tracking-widest leading-tight">BY DRIVE</h1>
              <p className="text-[10px] text-white/50 uppercase tracking-widest">Aggregator System</p>
            </div>
          </div>
      ...
      {/* BOTTOM NAV MOBILE: Muncul cuma di layar HP nempel di bawah */}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link 
              key={item.name} 
              href={item.path} 
              className={`flex flex-col items-center p-2 min-w-[72px] transition-colors ${
                isActive ? 'text-[#2A0510]' : 'text-gray-400'
              }`}
            >
